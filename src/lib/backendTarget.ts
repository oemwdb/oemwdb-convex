...
export const BACKEND_TARGET_DEFAULT_MIGRATION_KEY = "oemwdb_backend_target_workshop_default_v1";
...
export function defaultBackendTarget(): BackendTarget {
  const env = (import.meta as any).env ?? {};
  return normalizeBackendTarget(env.VITE_CONVEX_DEFAULT_TARGET || "workshop");
}

export function readStoredBackendTarget(): BackendTarget {
  const fallbackTarget = defaultBackendTarget();
  if (typeof window === "undefined") return fallbackTarget;

  const stored = window.localStorage.getItem(BACKEND_TARGET_STORAGE_KEY);

  if (
    fallbackTarget === "workshop" &&
    window.localStorage.getItem(BACKEND_TARGET_DEFAULT_MIGRATION_KEY) !== "1"
  ) {
    window.localStorage.setItem(BACKEND_TARGET_DEFAULT_MIGRATION_KEY, "1");
    window.localStorage.setItem(BACKEND_TARGET_STORAGE_KEY, "workshop");
    return "workshop";
  }

  return stored ? normalizeBackendTarget(stored) : fallbackTarget;
}
...
export function getBackendUrlConfig() {
  const env = (import.meta as any).env ?? {};
  const localUrl = trimValue(env.VITE_CONVEX_LOCAL_URL);
  const localSiteUrl = trimValue(env.VITE_CONVEX_LOCAL_SITE_URL);
  const controlUrl =
    trimValue(env.VITE_CONVEX_CONTROL_URL) ||
    trimValue(env.VITE_CONVEX_URL);
  const workshopUrl =
    trimValue(env.VITE_CONVEX_WORKSHOP_URL) ||
    localUrl;
