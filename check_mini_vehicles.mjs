...
async function checkMiniVehicles() {
    const { data: vehicles, error } = await supabase
        .from('oem_vehicles')
        .select('id, title, brand_ref, bolt_pattern, center_bore, series')
    ...
    const validSpecs = vehicles.filter(v =>
        v.bolt_pattern && v.bolt_pattern.length > 0 &&
        v.center_bore && v.center_bore.length > 0
    );
    ...
    if (vehicles.length > 0) {
        console.log("\nSample Vehicles:");
        vehicles.slice(0, 5).forEach(v => console.log(v));
    }
}
