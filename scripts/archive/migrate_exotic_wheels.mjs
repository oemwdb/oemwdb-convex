        ...
        body: JSON.stringify({
            filter: {
                and: [
                    ...
                    {
                        or: [
                            { property: "Brand Relation", rich_text: { contains: "Ferrari" } },
                            { property: "Brand Relation", rich_text: { contains: "Lamborghini" } }
                        ]
                    }
                ]
            },
...
async function processWheel(page, index, total) {
    ...
    console.log(`\n[${index + 1}/${total}] Processing: ${wheel.wheel_name} (${wheel.brand})`);
...
// Main migration
async function main() {
    console.log('🚀 Starting Ferrari & Lamborghini wheels migration...\n');
