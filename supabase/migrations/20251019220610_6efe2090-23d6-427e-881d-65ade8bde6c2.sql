-- Drop and recreate get_vehicles_with_brands function without DISTINCT to fix ORDER BY error
DROP FUNCTION IF EXISTS public.get_vehicles_with_brands();

CREATE OR REPLACE FUNCTION public.get_vehicles_with_brands()
 RETURNS TABLE(id text, chassis_code text, model_name text, vehicle_title text, brand_name text, production_years text, bolt_pattern text, center_bore text, vehicle_image text, status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
