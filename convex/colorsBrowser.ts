...
export const collectionList = query({
  args: {},
  handler: async (ctx) => {
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      wheelVariants: wheelVariantDocs.map((variant) => ({
        ...
        size: [normalizeColorText(variant.diameter), normalizeColorText(variant.width)]
          .filter(Boolean)
          .join(" x ") || null,
        ...
        center_bore: normalizeColorText(variant.center_bore) || null,
        weight: normalizeColorText(variant.weight) || null,
        tire_size: normalizeColorText(variant.tire_size) || null,
